...
iftDataSet *ExtractMarkerDataSet(iftMImage *img, iftLabeledSet *S,
                                 iftAdjRel *A) {
...
iftLabeledSet *SelectClusterCenters(iftImage *img, iftLabeledSet *S,
                                    float adj_radius,
  ...
  for (int c = 1; c <= Z->nclasses; c++) {
    ...
    for (int s = 0; s < Z1->nsamples; s++) {
      if (iftHasSampleStatus(Z1->sample[s], IFT_PROTOTYPE)) {
        ...
        iftInsertLabeledSetMarkerAndHandicap(&T, p, labels->val[p],
                                             markers->val[p], 0);
      }
    }
